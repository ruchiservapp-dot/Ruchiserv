...
class InventoryRepository {
  ...
  Future<List<Map<String, dynamic>>> getIngredientsMaster(String firmId) async {
    ...
    final res = await db.query('firms',
        ...
        where: 'firmId = ?',
        whereArgs: [firmId]);
    bool showUniversal = res.isNotEmpty
        ? (res.first['showUniversalData'] as int? ?? 1) == 1
        : true;
  ...
  Future<List<Map<String, dynamic>>> getAllDishes(String firmId) async {
    final db = await _dbHelper.database;
    final showUniversal =
        await _dbHelper.getFirmUniversalDataVisibility(firmId);
    if (showUniversal) {
      return await db.query('dish_master',
          where: 'firmId = ? OR firmId = ?',
          whereArgs: [firmId, 'SEED'],
          orderBy: 'name');
    } else {
      return await db.query('dish_master',
  ...
  Future<List<Map<String, dynamic>>> getBomForDish(
      String firmId, int dishId) async {
    final db = await _dbHelper.database;
    ...
    if (result.isEmpty) {
      final dish =
          await db.query('dish_master', where: 'id = ?', whereArgs: [dishId]);
      if (dish.isNotEmpty) {
  ...
  Future<List<Map<String, dynamic>>> getRecipeForDishByName(
      String dishName, int paxQty) async {
    final db = await _dbHelper.database;
    final sp = await SharedPreferences.getInstance();
    final firmId = sp.getString('last_firm') ?? 'DEFAULT';

    final resF = await db.query('firms',
        columns: ['showUniversalData'],
        where: 'firmId = ?',
        whereArgs: [firmId]);
    bool showUniversal = resF.isNotEmpty
        ? (resF.first['showUniversalData'] as int? ?? 1) == 1
        : true;

    var where =
        "name = ? AND (firmId = ? ${showUniversal ? "OR firmId = 'SEED'" : ""})";
    var args = <Object>[dishName.trim(), firmId];

    var dishMaster = await db.query('dish_master',
    ...
    if (dishMaster.isEmpty) {
      where =
          "name LIKE ? AND (firmId = ? ${showUniversal ? "OR firmId = 'SEED'" : ""})";
      args = ['%${dishName.trim()}%', firmId];
      dishMaster = await db.query('dish_master',
          columns: ['id', 'baseId', 'base_pax', 'firmId', 'name'],
          where: where,
          whereArgs: args,
          orderBy: "CASE WHEN firmId = '$firmId' THEN 0 ELSE 1 END",
  ...
  Future<int> insertBomItem(Map<String, dynamic> data) async {
    final cloudSync = CloudSyncService();
    final qtyPer100 = (data['quantityPer100Pax'] as num?) ?? 0;
    final bomData = {
      'firmId': data['firmId'] ?? 'SEED',
      'dish_id': data['dishId'] ?? data['dish_id'],
      'ing_id': data['ingredientId'] ?? data['ing_id'],
      'quantity_per_base_pax':
          data['quantity_per_base_pax'] ?? (qtyPer100 / 100.0),
      'unit_override': data['unit'] ?? data['unit_override'],
      'isModified': 1,
      'uuid': data['uuid'] ?? const Uuid().v4(),
    };
    final id =
        await cloudSync.awsFirstWrite(table: 'recipe_detail', data: bomData);
  ...
  Future<bool> deleteBomItem(int id) async {
    await CloudSyncService()
        .awsFirstDelete(table: 'recipe_detail', recordId: id);
  ...
  Future<int?> createMrpRun(Map<String, dynamic> data) async {
    ...
    int runNumber =
        (existingRuns.isNotEmpty && existingRuns.first['maxNum'] != null)
            ? (existingRuns.first['maxNum'] as int) + 1
            : 1;
  ...
  Future<List<Map<String, dynamic>>> getMrpRuns(String firmId) async {
    ...
    return await db.query('mrp_runs',
        where: 'firmId = ?', whereArgs: [firmId], orderBy: 'createdAt DESC');
  }
  ...
  Future<void> updateMrpOutputAllocation(
    ...
    final db = await _dbHelper.database;
    final qty = (await db.query('mrp_output',
            columns: ['requiredQty'],
            where: 'id = ?',
      ...
      if (outputs.isNotEmpty) {
        batch.update(
            'mrp_output',
            {
              ...
              'allocationStatus': supplierId != null ? 'ALLOCATED' : 'PENDING',
              'allocatedQty':
                  supplierId != null ? outputs.first['requiredQty'] : 0,
            },
            where: 'id = ?',
            whereArgs: [outputs.first['id']]);
      }
    }
    await batch.commit(noResult: true);
  }
  ...
  Future<Map<int, int?>> getExistingAllocations(int mrpRunId) async {
    ...
    final results = await db.query('mrp_output',
        ...
        whereArgs: [mrpRunId]);
    return Map.fromEntries(results.map(
    ...
    for (var orderId in orderIds) {
      ...
      if (existing.isNotEmpty) {
        ...
        if (curId == null || curStatus == 'PENDING' || curStatus == null) {
          ...
          await db.update('orders', {'isLocked': 1, 'lockedAt': now},
  ...
  Future<bool> safeResetOrderForMRP(int orderId) async {
    ...
    final order = await db.query('orders',
        columns: ['mrpRunId'], where: 'id = ?', whereArgs: [orderId]);
    ...
    if (mrpRunId != null) {
      ...
      await db.update('mrp_output', {'allocationStatus': 'CANCELLED'},
  ...
  Future<int?> runMrpInTransaction(
      ...
      required Future<Map<int, Map<String, dynamic>>> Function(
              List<int> orderIds)
          calculateOutput}) async {
    final db = await _dbHelper.database;
    try {
      final result = await db.transaction((txn) async {
        ...
        for (var oid in orderIds) {
          ...
          if (check.first['mrpStatus'] != null &&
              check.first['mrpStatus'] != 'PENDING' &&
              check.first['mrpRunId'] != null)
            throw Exception('Order $oid already processed');
        }
        final monthStart =
            DateTime(now.year, now.month, 1).toIso8601String().substring(0, 10);
        ...
        final existingRuns = await txn.rawQuery(
            'SELECT MAX(runNumber) as maxNum FROM mrp_runs WHERE firmId = ? AND date(runDate) >= date(?) AND date(runDate) <= date(?)',
            [firmId, monthStart, monthEnd]);
        int runNumber =
            (existingRuns.isNotEmpty && existingRuns.first['maxNum'] != null)
                ? (existingRuns.first['maxNum'] as int) + 1
        ...
        for (var oid in orderIds) {
          await txn.insert(
              'mrp_run_orders', {'mrpRunId': mrpRunId, 'orderId': oid},
        ...
        for (var entry in output.entries) {
          ...
          item['requiredQty'] =
              roundByCategory(qty, item['category'] as String?);
          batch.insert('mrp_output', {'mrpRunId': mrpRunId, ...item});
        }
        await batch.commit(noResult: true);
