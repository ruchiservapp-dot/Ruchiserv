...
class OperationRepository {
  ...
  Future<List<Map<String, dynamic>>> getAllStaff({bool onlyActive = true}) async {
  ...
  Future<Map<String, dynamic>?> getStaffByMobile(String mobile, {String? firmId}) async {
    final db = await _dbHelper.database;
    final results = await db.query('staff',
      where: firmId != null ? 'mobile = ? AND firmId = ? AND isActive = 1' : 'mobile = ? AND isActive = 1',
  ...
  Future<void> updateAttendance(int staffId, String date, String status, {double? overtime}) async {
    final db = await _dbHelper.database;
    ...
    final existing = await db.query('attendance', where: 'staffId = ? AND date = ?', whereArgs: [staffId, date]);
    if (existing.isNotEmpty) {
      await cloudSync.awsFirstUpdate(table: 'attendance', recordId: existing.first['id'] as int, data: data);
    } else {
  ...
  Future<List<Map<String, dynamic>>> getDrivers(String firmId) async {
    final db = await _dbHelper.database;
    return await db.query('users', where: 'role = ? AND firmId = ?', whereArgs: ['Driver', firmId]);
  }
  ...
  Future<bool> updateStaffFields(int id, Map<String, dynamic> updates) async {
    ...
    final success = await cloudSync.awsFirstUpdate(table: 'staff', recordId: id, data: updates);
  ...
  Future<bool> deleteStaff(int id) async {
    final cloudSync = CloudSyncService();
    final success = await cloudSync.awsFirstDelete(table: 'staff', recordId: id);
  ...
  Future<bool> updateAttendanceRecord(int id, Map<String, dynamic> updates) async {
    final cloudSync = CloudSyncService();
    updates['id'] = id;
    final success = await cloudSync.awsFirstUpdate(table: 'attendance', recordId: id, data: updates);
    AppLogger.success('✅ [Operation] Updated attendance #$id (AWS-first)');
    return success;
  }

  Future<List<Map<String, dynamic>>> getAttendanceForStaff(int staffId, DateTime startDate, DateTime endDate) async {
    final db = await _dbHelper.database;
    final startStr = startDate.toIso8601String().substring(0, 10);
    final endStr = endDate.toIso8601String().substring(0, 10);
    return await db.rawQuery('''
      SELECT date, checkIn, checkOut, hoursWorked, overtime, status, isWithinGeoFence
      FROM attendance
  ...
  Future<List<Map<String, dynamic>>> getHRAttendanceReport(String startDate, String endDate, String firmId) async {
    final db = await _dbHelper.database;
    return await db.rawQuery('''
      SELECT s.id, s.name, s.role, s.staffType, COUNT(a.id) as daysPresent,
             COALESCE(SUM(a.hoursWorked), 0) as totalHours, COALESCE(SUM(a.overtime), 0) as totalOvertime,
  ...
  Future<List<Map<String, dynamic>>> getHROvertimeReport(String startDate, String endDate, String firmId) async {
    final db = await _dbHelper.database;
    return await db.rawQuery('''
      SELECT s.id, s.name, s.hourlyRate, COALESCE(SUM(a.overtime), 0) as totalOT,
             COALESCE(SUM(a.overtime), 0) * COALESCE(s.hourlyRate, 0) as otPay
      FROM staff s LEFT JOIN attendance a ON s.id = a.staffId AND a.date BETWEEN ? AND ? AND a.overtime > 0
      WHERE s.isActive = 1 AND s.firmId = ? GROUP BY s.id HAVING totalOT > 0 ORDER BY totalOT DESC
    ''', [startDate, endDate, firmId]);
  }

  Future<List<Map<String, dynamic>>> getDispatchReport(String startDate, String endDate, String firmId) async {
    final db = await _dbHelper.database;
    return await db.rawQuery('''
      SELECT dispatchStatus as status, COUNT(*) as count, SUM(COALESCE(kmForward, 0) + COALESCE(kmReturn, 0)) as totalKm
      FROM dispatches d
  ...
  Future<bool> toggleDishSubcontract(int dishId, bool isSubcontracted) async {
    ...
    return await cloudSync.awsFirstUpdate(table: 'dishes', recordId: dishId, data: data);
  }
  ...
  Future<bool> updateVehicle(int id, Map<String, dynamic> data) async {
    ...
    final success = await cloudSync.awsFirstUpdate(table: 'vehicles', recordId: id, data: data);
  ...
  Future<bool> deleteVehicle(int id) async {
    final cloudSync = CloudSyncService();
    final success = await cloudSync.awsFirstDelete(table: 'vehicles', recordId: id);
    AppLogger.success('✅ [Operation] Deleted vehicle #$id (AWS-first)');
    return success;
  }

  Future<List<Map<String, dynamic>>> getAllVehicles({bool onlyActive = true}) async {
    final db = await _dbHelper.database;
    if (onlyActive) {
      return await db.query('vehicles', where: 'isActive = 1', orderBy: 'vehicleNumber');
    }
  ...
  Future<int?> insertDispatchItem(Map<String, dynamic> data) async {
    final cloudSync = CloudSyncService();
    data['uuid'] = data['uuid'] ?? _generateUuid();
    final id = await cloudSync.awsFirstWrite(table: 'dispatch_items', data: data);
  ...
  Future<void> updateDispatchAssignment(int dispatchId, String status, {String? rejectionReason}) async {
    ...
    if (status == 'ACCEPTED') updates['acceptedAt'] = now;
      ...
      updates['rejectedAt'] = now; updates['rejectionReason'] = rejectionReason; updates['driverId'] = null;
    }
    await cloudSync.awsFirstUpdate(table: 'dispatches', recordId: dispatchId, data: updates);
    AppLogger.success('✅ [Operation] Updated assignment #$dispatchId to $status (AWS-first)');
  }

  Future<void> updateDispatchKmAndEarnings(int dispatchId, {double? kmForward, double? kmReturn, double? driverShare}) async {
    final cloudSync = CloudSyncService();
    ...
    await cloudSync.awsFirstUpdate(table: 'dispatches', recordId: dispatchId, data: updates);
    AppLogger.success('✅ [Operation] Updated km/earnings for #$dispatchId (AWS-first)');
  }
  ...
  Future<List<Map<String, dynamic>>> getSubcontractorDishes(int subcontractorId, int orderId) async {
    final db = await _dbHelper.database;
    return await db.query('dishes', where: 'orderId = ? AND isSubcontracted = 1 AND subcontractorId = ?', whereArgs: [orderId, subcontractorId]);
  }
  ...
  Future<void> updateOrderServiceAssignment(int orderId, {int? serviceSubId, int? counterSubId}) async {
    ...
    if (updates.length > 1) {
      ...
      AppLogger.success('✅ [Operation] Updated service assignments for order #$orderId (AWS-first)');
    }
  }

  Future<void> assignDriverToDispatch(int dispatchId, int driverId) async {
    final db = await _dbHelper.database;
    await db.update('dispatches', {'driverId': driverId, 'assignmentStatus': 'PENDING', 'assignedAt': DateTime.now().toIso8601String()}, where: 'id = ?', whereArgs: [dispatchId]);
  }

  Future<bool> updateDispatchItem(int id, Map<String, dynamic> data) async {
    final cloudSync = CloudSyncService();
    data['id'] = id;
    final success = await cloudSync.awsFirstUpdate(table: 'dispatch_items', recordId: id, data: data);
    AppLogger.success('✅ [Operation] Updated dispatch item #$id (AWS-first)');
    return success;
  }

  Future<bool> updateUtensilByName(String name, Map<String, dynamic> data) async {
    final db = await _dbHelper.database;
    final res = await db.query('utensils', where: 'name = ?', whereArgs: [name]);
  ...
  Future<bool> updateDispatch(int id, Map<String, dynamic> data) async {
    final cloudSync = CloudSyncService();
    data['id'] = id;
    final success = await cloudSync.awsFirstUpdate(table: 'dispatches', recordId: id, data: data);
  ...
  Future<List<Map<String, dynamic>>> getDispatchItems(int dispatchId) async {
    final db = await _dbHelper.database;
    return await db.query('dispatch_items', where: 'dispatchId = ?', whereArgs: [dispatchId]);
  }
  ...
  Future<Map<String, dynamic>?> getFirmDetails(String firmId) async {
    final db = await _dbHelper.database;
    final results = await db.query('firms', where: 'firmId = ?', whereArgs: [firmId]);
    return results.isNotEmpty ? results.first : null;
  }
  ...
  Future<Map<String, dynamic>?> getSalarySlipData(int staffId, String monthYear) async {
    ...
    final attendance = await db.rawQuery('''
      SELECT COUNT(*) as daysPresent, COALESCE(SUM(hoursWorked), 0) as totalHours, COALESCE(SUM(overtime), 0) as totalOvertime
      FROM attendance
      WHERE staffId = ? AND date BETWEEN ? AND ? AND status = 'Present'
    ''', [staffId, startDate, endDate]);
    ...
    final disbursement = await db.query('salary_disbursements',
      where: 'staffId = ? AND monthYear = ?', whereArgs: [staffId, monthYear], limit: 1);
  ...
  Future<List<Map<String, dynamic>>> getStaffSalaryHistory(int staffId, {int limit = 12}) async {
    ...
    return await db.query('salary_disbursements',
      where: 'staffId = ?', whereArgs: [staffId], orderBy: 'monthYear DESC', limit: limit);
  }
