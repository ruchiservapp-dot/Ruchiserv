...
class OrderRepository {
  ...
  Future<int?> insertOrder(
    ...
    AppLogger.success('✅ [Order] Created order #$orderId with ${dishes.length} dishes (AWS-first)');
    return orderId;
  }
  ...
  Future<bool> updateOrder(
    ...
    AppLogger.success('✅ [Order] Updated order #$orderId with ${dishes.length} dishes (AWS-first)');
    return true;
  }

  Future<void> updateOrderFields(int orderId, Map<String, dynamic> updates) async {
    final cloudSync = CloudSyncService();
    updates['id'] = orderId;
  ...
  Future<Map<String, dynamic>?> getFirm(String firmId) async {
    final db = await _dbHelper.database;
    final res = await db.query('firms', where: 'firmId = ?', whereArgs: [firmId], limit: 1);
    return res.isNotEmpty ? res.first : null;
  }

  Future<void> upsertServiceRate(String firmId, String serviceType, double rate) async {
    final db = await _dbHelper.database;
    final cloudSync = CloudSyncService();
    
    final existing = await db.query('service_rates', where: 'firmId = ? AND serviceType = ?', whereArgs: [firmId, serviceType]);
    final data = {
      'firmId': firmId,
    ...
    if (existing.isNotEmpty) {
      await cloudSync.awsFirstUpdate(table: 'service_rates', recordId: existing.first['id'] as int, data: data);
    } else {
  ...
  Future<bool> deleteOrder(int orderId) async {
    ...
    AppLogger.success('✅ [Order] Deleted order #$orderId and ${existingDishes.length} dishes (AWS-first)');
    return success;
  }

  Future<List<Map<String, dynamic>>> getOrdersByDate(String date, String firmId) async {
    final db = await _dbHelper.database;
    return db.query(
      'orders',
  ...
  Future<List<Map<String, dynamic>>> getOrdersWithPax(String date, String firmId) async {
    final db = await _dbHelper.database;
  ...
  Future<List<Map<String, dynamic>>> getDishesForOrder(int orderId, String firmId) async {
    final db = await _dbHelper.database;
    return await db.rawQuery('''
          ...
          if (orderId != null && clientStatus != null) {
            await db.update(
              'orders',
              {
                ...
                if (awsOrder['confirmedAt'] != null) 'confirmedAt': awsOrder['confirmedAt'],
                if (awsOrder['changeRequestedAt'] != null) 'changeRequestedAt': awsOrder['changeRequestedAt'],
  ...
  Future<List<Map<String, dynamic>>> getPendingOrdersForMrp(String date, String firmId) async {
    final db = await _dbHelper.database;
  ...
  Future<List<Map<String, dynamic>>> getProcessedOrdersForMrp(String date, String firmId) async {
    final db = await _dbHelper.database;
    return await db.query('orders',
      where: 'date = ? AND mrpStatus != "PENDING" AND firmId = ?', 
      whereArgs: [date, firmId]);
  ...
  Future<Map<String, dynamic>> getOrderDependencies(int orderId, String firmId) async {
    ...
    return {'order': order, 'dishCount': dishes.length, 'hasDispatch': dispatchCount > 0, 'dispatchCount': dispatchCount};
  }
  ...
  Future<bool> cancelOrder(int orderId, {required String firmId, required String userId}) async {
    try {
      final db = await _dbHelper.database;
      final now = DateTime.now().toIso8601String();
      await db.update('orders', {'isCancelled': 1, 'cancelledAt': now, 'cancelledBy': userId, 'status': 'CANCELLED', 'updatedAt': now}, where: 'id = ? AND firmId = ?', whereArgs: [orderId, firmId]);
      _dbHelper.syncStreamController.add(SyncEvent(table: 'orders', data: {'id': orderId, 'isCancelled': 1, 'cancelledAt': now, 'cancelledBy': userId, 'status': 'CANCELLED', 'updatedAt': now}, action: 'UPDATE', filters: {'id': orderId}));
      return true;
    } catch (e) { return false; }
  }

  Future<List<Map<String, dynamic>>> getDishesSummaryByDate(String date, String firmId) async {
    final db = await _dbHelper.database;
    return await db.rawQuery('''
  ...
  Future<List<Map<String, dynamic>>> getDishesForDate(String date, String firmId) async {
    final db = await _dbHelper.database;
    return await db.rawQuery('''
  ...
  Future<List<Map<String, dynamic>>> getDishSuggestions(String? category) async {
    try {
      ...
      if (category != null && category.isNotEmpty) {
        String pattern;
        switch (category) {
          case 'Starters': pattern = 'Starter%'; break;
          case 'Main Course': pattern = 'Main Course%'; break;
          case 'Desserts': pattern = 'Dessert%'; break;
          case 'Beverages': pattern = 'Beverage%'; break;
          case 'Specialties': pattern = 'Special%'; break;
          default: pattern = '$category%';
        }
        whereClause += " AND category LIKE ?";
        args.add(pattern);
        return await db.query('dish_master', where: whereClause, whereArgs: args, orderBy: 'name ASC');
      }
      return await db.query('dish_master', where: whereClause, whereArgs: args, orderBy: 'category, name ASC');
    } catch (_) { return []; }
  }

  Future<void> upsertDishMaster({required String name, required String category, required int rate, String foodType = 'Veg'}) async {
    ...
    try {
      final firmDish = await db.query('dish_master', where: 'name = ? AND category = ? AND firmId = ?', whereArgs: [name.trim(), category, firmId], limit: 1);
      if (firmDish.isNotEmpty) {
        await db.update('dish_master', {'rate': rate, 'foodType': foodType, 'updatedAt': now, 'isModified': 1}, where: 'id = ?', whereArgs: [firmDish.first['id']]);
      } else {
        final seedDish = await db.query('dish_master', where: 'name = ? AND category = ? AND firmId = ?', whereArgs: [name.trim(), category, 'SEED'], limit: 1);
        if (seedDish.isNotEmpty) {
          final s = seedDish.first;
          if ((s['rate'] as num).toInt() != rate || s['foodType'] as String != foodType) {
            await db.insert('dish_master', {'firmId': firmId, 'baseId': s['id'], 'name': name.trim(), 'category': category, 'rate': rate, 'foodType': foodType, 'createdAt': now, 'updatedAt': now, 'isModified': 1});
          }
        } else {
          await db.insert('dish_master', {'firmId': firmId, 'name': name.trim(), 'category': category, 'rate': rate, 'foodType': foodType, 'createdAt': now, 'updatedAt': now, 'isModified': 1});
        }
      }
    } catch (e) { AppLogger.error('❌ Error upserting dish master for $name: $e'); }
  }
  ...
  Future<List<Map<String, dynamic>>> getOrderStatusReport(String startDate, String endDate, String firmId) async {
    final db = await _dbHelper.database;
    return await db.rawQuery('''
  ...
  Future<List<Map<String, dynamic>>> getOrdersByFoodTypeReport(String startDate, String endDate, String firmId) async {
    final db = await _dbHelper.database;
    return await db.rawQuery('''
  ...
  Future<List<Map<String, dynamic>>> getOrdersByMealTypeReport(String startDate, String endDate, String firmId) async {
    final db = await _dbHelper.database;
    return await db.rawQuery('''
  ...
  Future<List<Map<String, dynamic>>> getKitchenProductionReport(String startDate, String endDate, String firmId) async {
    final db = await _dbHelper.database;
    return await db.rawQuery('''
  ...
  Future<List<Map<String, dynamic>>> getTopDishesReport(String startDate, String endDate, String firmId, {int limit = 10}) async {
    final db = await _dbHelper.database;
    return await db.rawQuery('''
  ...
  Future<List<Map<String, dynamic>>> getDishesByCategoryReport(String startDate, String endDate, String firmId) async {
    final db = await _dbHelper.database;
    return await db.rawQuery('''
  ...
  Future<List<Map<String, dynamic>>> getDeliveryTimeReport(String startDate, String endDate, String firmId) async {
    final db = await _dbHelper.database;
    return await db.rawQuery('''
  ...
  Future<List<Map<String, dynamic>>> getRevenueByLocationReport(String startDate, String endDate, String firmId) async {
    final db = await _dbHelper.database;
    return await db.rawQuery('''
  ...
  Future<List<Map<String, dynamic>>> getAllCustomers(String firmId) async {
    final db = await _dbHelper.database;
    return await db.query('customers', where: 'firmId = ?', whereArgs: [firmId], orderBy: 'name');
  }
  ...
  Future<List<Map<String, dynamic>>> getDailyCapacityReport(String startDate, String endDate, String firmId) async {
    final db = await _dbHelper.database;
    return await db.rawQuery('''
  ...
  Future<List<Map<String, dynamic>>> getOrdersByFilter(String key, String value, {required String startDate, required String endDate, required String firmId}) async {
    final db = await _dbHelper.database;
    return await db.query('orders',
