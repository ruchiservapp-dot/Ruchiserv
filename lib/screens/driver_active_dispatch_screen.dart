...
class _DriverActiveDispatchScreenState extends State<DriverActiveDispatchScreen> {
  ...
  void initState() {
    super.initState();
    AppLogger.info("📍 DriverActiveDispatchScreen: initState start for dispatch ${widget.dispatch['id']}");
    ...
    _loadDetails();
    AppLogger.info("📍 DriverActiveDispatchScreen: initState end");
    ...
    _syncSubscription = DatabaseHelper().syncStreamController.stream.listen((event) {
      if (['dispatches', 'orders', 'dispatch_items'].contains(event.table)) {
        AppLogger.info('⚡ DriverActiveDispatchScreen: Real-time update detected for ${event.table}. Refreshing...');
        _loadDetails();
      }
    });
  }
  ...
  Future<void> _loadDetails() async {
    AppLogger.info("📍 DriverActiveDispatchScreen: _loadDetails start");
    setState(() => _isLoading = true);
    
    try {
      final db = await DatabaseHelper().database;
      AppLogger.info("📍 DriverActiveDispatchScreen: DB initialized");
      ...
      if (dispatches.isNotEmpty) {
        ...
        AppLogger.info("📍 DriverActiveDispatchScreen: Dispatch refreshed: ${_dispatch['id']}");
      }
      ...
      if (orders.isNotEmpty) {
        ...
        AppLogger.info("📍 DriverActiveDispatchScreen: Order loaded: ${_order['id']}");
      }
      
      // Get dispatch items
      final items = await db.query('dispatch_items', where: 'dispatchId = ?', whereArgs: [_dispatch['id']]);
      _items = List<Map<String, dynamic>>.from(items);
      AppLogger.info("📍 DriverActiveDispatchScreen: Items loaded: ${_items.length}");
      
      if (mounted) setState(() => _isLoading = false);
      AppLogger.info("📍 DriverActiveDispatchScreen: _loadDetails complete, loading=false");
    } catch (e, stack) {
      AppLogger.info("📍 DriverActiveDispatchScreen error: $e");
      print(stack);
      if (mounted) setState(() => _isLoading = false);
    }
  }
  ...
  Future<void> _advanceStage() async {
    ...
    if (confirmed == true) {
      ...
      await OperationRepository().updateDispatch(_dispatch['id'], updates);
  ...
  Future<void> _postLocation() async {
    ...
    try {
        ...
        if (loc != null) {
            AppLogger.info("📍 Posting Location: ${loc.latitude}, ${loc.longitude}");
        ...
        AppLogger.info("⚠️ Location Post Failed: $e");
    }
  }
