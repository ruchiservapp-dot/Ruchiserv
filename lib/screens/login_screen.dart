...
import 'policies/contact_us_screen.dart';
import 'policies/terms_conditions_screen.dart';
import 'policies/pricing_screen.dart';
import 'policies/products_services_screen.dart';
import 'policies/refund_cancellation_screen.dart';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _initAsync() async {
    AppLogger.info('LoginDebug: _initAsync started');
    await _checkBiometric();
    await _prefill();
  }

  Future<void> _prefill() async {
    AppLogger.info('LoginDebug: _prefill started');
    ...
    if (pendingFirmId != null && pendingFirmId.isNotEmpty) {
      AppLogger.info('LoginDebug: Found pending credentials');
    ...
    AppLogger.info('LoginDebug: LastFirm: $lastFirm, LastMobileBio: $lastMobileBiometric');
    ...
    AppLogger.info('LoginDebug: bioEnabled: $bioEnabled, bioAvailable: $_biometricAvailable');
    
    if (bioEnabled && _biometricAvailable) {
      AppLogger.info('LoginDebug: Triggering biometric login...');
  ...
  Future<void> _checkBiometric() async {
    try {
      ...
      AppLogger.info('LoginDebug: _checkBiometric result: $ok');
      if (!mounted) return;
      setState(() => _biometricAvailable = ok);
    } catch (e) {
      AppLogger.info('LoginDebug: _checkBiometric error: $e');
      if (!mounted) return;
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (online) {
        ...
        if (!allowed) {
          AppLogger.info('LoginScreen: Online login failed, trying offline fallback...');
      ...
      if (userList.isNotEmpty) {
        ...
        if (existingRole == null || existingRole.isEmpty || existingRole == 'Staff') {
          ...
          AppLogger.info('LoginScreen: No user in DB, defaulting to Admin for first login');
        }
      }
  ...
  Future<void> _loginWithBiometric() async {
    ...
    try {
      ...
      AppLogger.info('LoginDebug: Authentication Success');

      // Offline-with-biometric rule
      AppLogger.info('LoginDebug: Checking canLoginOfflineWithBiometric...');
      final can = await AuthService.canLoginOfflineWithBiometric(firmId: firmId);
      AppLogger.info('LoginDebug: canLoginOfflineWithBiometric: $can');
      if (!can) {
        if (!mounted) return;
        setState(() => _isLoading = false);
      ...
      AppLogger.info('LoginDebug: Checking Expiry...');
      if (await AuthService.isExpired()) {
        if (!mounted) return;
        _showExpiryDialog(lock: true);
        return;
      ...
      if (!mounted) return;
      AppLogger.info('LoginDebug: Navigating to MainMenuScreen...');
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => const MainMenuScreen()),
      );
      AppLogger.info('LoginDebug: Navigation triggered');
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context)
  ...
  Widget _buildPolicyLink(BuildContext context, String title, Widget screen) {
    return TextButton(
      onPressed: () {
        Navigator.push(
          context,
        ...
        child: Center(
          child: ConstrainedBox(
            ...
            child: ShakeAnimation(
              shake: _shouldShake,
              onComplete: () => setState(() => _shouldShake = false),
              child: Form(
                key: _formKey,
                child: ListView(
                  shrinkWrap: true,
                  ...
                  FutureBuilder<bool>(
                    ...
                    // Cashfree KYC Policy Links
                    Wrap(
                      alignment: WrapAlignment.center,
                      spacing: 8,
                      runSpacing: 4,
                      children: [
                        _buildPolicyLink(context, 'Contact Us', const ContactUsScreen()),
                        _buildPolicyLink(context, 'Terms & Conditions', const TermsConditionsScreen()),
                        _buildPolicyLink(context, 'Pricing', const PricingScreen()),
                        _buildPolicyLink(context, 'Products & Services', const ProductsServicesScreen()),
                        _buildPolicyLink(context, 'Refund Policy', const RefundCancellationScreen()),
                      ],
                    ),
                    const SizedBox(height: 24),
                  ],
