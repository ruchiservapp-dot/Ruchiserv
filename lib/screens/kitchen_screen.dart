...
class _KitchenScreenState extends State<KitchenScreen>
  ...
  final ScrollController _dateScrollController =
      ScrollController(); // Date scroller controller
  ...
  void initState() {
    ...
    _syncSubscription =
      ...
      if (['orders', 'dishes'].contains(event.table)) {
        AppLogger.info(
            '⚡ KitchenScreen: Real-time update detected for ${event.table}. Refreshing...');
        _loadData();
      }
    });
  ...
  Future<void> _loadData() async {
    AppLogger.info('🔄 KitchenScreen: Setting isLoading = true');
    setState(() => _isLoading = true);

    // Load each tab independently so one failure doesn't block others
    try {
      await _loadOrdersForDate();
    } catch (e) {
      AppLogger.error('❌ KitchenScreen: Failed to load orders: $e');
    }

    try {
      await _loadProductionQueue();
    } catch (e) {
      AppLogger.error('❌ KitchenScreen: Failed to load production queue: $e');
    }

    try {
      await _loadReadyQueue();
    } catch (e) {
      AppLogger.error('❌ KitchenScreen: Failed to load ready queue: $e');
    }
  ...
  Future<void> _loadOrdersForDate() async {
    ...
    for (var o in orders) {
      final dishes = await OrderRepository().getDishesForOrder(o['id'], firmId);
  ...
  Future<void> _updateDish(int id, Map<String, dynamic> updates) async {
    ...
    await OrderRepository().updateDish(id, updates);
  ...
  Widget ordersList(Map<String, List<Map<String, dynamic>>> grouped) {
    ...
    final sections = [
      if (grouped['Breakfast']!.isNotEmpty)
        ...grouped['Breakfast']!.map((o) => _buildOrderCard(o)),
      if (grouped['Lunch']!.isNotEmpty)
        ...grouped['Lunch']!.map((o) => _buildOrderCard(o)),
      if (grouped['Dinner']!.isNotEmpty)
        ...grouped['Dinner']!.map((o) => _buildOrderCard(o)),
      if (grouped['Other']!.isNotEmpty)
        ...grouped['Other']!.map((o) => _buildOrderCard(o)),
    ];
    // On tablet/desktop: 2-column grid; on phone: single column
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return ListView(
          ...
          children: [
            if (grouped['Breakfast']!.isNotEmpty)
              _buildMealSection(AppLocalizations.of(context).breakfast,
                  grouped['Breakfast']!),
            if (grouped['Lunch']!.isNotEmpty)
              _buildMealSection(
                  AppLocalizations.of(context).lunch, grouped['Lunch']!),
            if (grouped['Dinner']!.isNotEmpty)
              _buildMealSection(
                  AppLocalizations.of(context).dinner, grouped['Dinner']!),
            if (grouped['Other']!.isNotEmpty)
              _buildMealSection(
                  AppLocalizations.of(context).other, grouped['Other']!),
          ],
        );
      },
    );
          ...
          children: [
            Row(
              ...
              children: [
                ...
                Text(TimeUtils.formatTo12Hour(order['time']),
                    style: const TextStyle(
            ...
            Text(
                "${AppLocalizations.of(context).paxLabel(order['totalPax'] ?? 0)} | ${AppLocalizations.of(context).locLabel(order['location'] ?? AppLocalizations.of(context).na)}",
  ...
  Widget _buildDishRow(Map<String, dynamic> dish) {
    ...
    if (status == 'COMPLETED')
      bgColor = Colors.green.shade50; // Override if done
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
            color: type == 'LIVE' ? Colors.orange : Colors.grey.shade200,
            width: type == 'LIVE' ? 2 : 1),
      ),
      child: Column(
        children: [
          Row(
            children: [
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      children: [
                        ...
                        if (type == 'LIVE')
                          Container(
                            ...
                            child: Text(AppLocalizations.of(context).live,
                                style: const TextStyle(
                                    fontSize: 10,
                                    fontWeight: FontWeight.bold,
                                    color: Colors.white)),
                    ...
                    Row(
                      children: [
                        Text(
                            "${AppLocalizations.of(context).qty}: ${dish['pax']}",
              ...
              if (status == 'PENDING' && (type == 'INTERNAL' || type == 'LIVE'))
                ElevatedButton(
                  onPressed: () =>
                      _updateDish(dish['id'], {'productionStatus': 'QUEUED'}),
                  style: ElevatedButton.styleFrom(
                    backgroundColor:
                        type == 'LIVE' ? Colors.deepOrange : Colors.orange,
                    padding: const EdgeInsets.symmetric(horizontal: 12),
                    minimumSize: const Size(60, 30),
                  ),
                  child: Text(
                      type == 'LIVE'
                          ? AppLocalizations.of(context).prep
                ...
                Text(
                    type == 'LIVE'
                        ? AppLocalizations.of(context).prepping
          ...
          if (status != 'COMPLETED')
            Align(
              ...
              child: TextButton(
                ...
                child: Text(AppLocalizations.of(context).assignEdit,
  ...
  void _showDishOptions(Map<String, dynamic> dish) async {
    ...
    final isLocked = await OrderRepository().isDishLocked(dish['id']);
    if (!mounted) return;
        ...
        builder: (_) {
          return Padding(
            ...
            child: Column(
              ...
              children: [
                Text(AppLocalizations.of(context).productionSettings,
                    style: const TextStyle(
      ...
      children: sortedDates.expand((date) {
        return [
          ...
          ...grouped[date]!.map((d) {
            ...
            return Semantics(
              label:
                  '${AppLocalizations.of(context).productionQueue}: $dishName for ${d['customerName']}, $pax quantities',
              child: Card(
                elevation: 3,
                margin: const EdgeInsets.only(bottom: 12),
                child: ExpansionTile(
                        ...
                        children: [
                          Container(
                            ...
                            decoration: BoxDecoration(
                                ...
                                borderRadius: BorderRadius.circular(4)),
                            child: Text(
                                "${AppLocalizations.of(context).qty}: $pax",
                                style: TextStyle(
                  ...
                  trailing: d['productionStatus'] == 'PENDING'
                      ...
                      : ElevatedButton.icon(
                          onPressed: () => _updateDish(
                              d['id'], {'productionStatus': 'COMPLETED'}),
                  ...
                  children: [
                    FutureBuilder<List<Map<String, dynamic>>>(
                      future: InventoryRepository()
                          .getRecipeForDishByName(dishName, pax),
                      builder: (context, snapshot) {
                        ...
                        if (recipe.isEmpty) {
                          return Padding(
                            padding: const EdgeInsets.all(8),
                            child: Text(
                                AppLocalizations.of(context).noRecipeDefined,
                                style: const TextStyle(
                                    color: Colors.grey,
          ...
          ...grouped[date]!.map((d) {
            ...
            return Card(
              ...
              child: ListTile(
                ...
                title: Text(d['dishName'] ?? d['name'] ?? 'Dish',
                    style: const TextStyle(fontWeight: FontWeight.bold)),
                subtitle: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                      ...
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                              ...
                              borderRadius: BorderRadius.circular(4)),
                          child: Text(
                              "${AppLocalizations.of(context).qty}: ${d['pax']}",
                              style: TextStyle(
                ...
                trailing: ElevatedButton.icon(
                  onPressed: () =>
                      _updateDish(d['id'], {'productionStatus': 'QUEUED'}),
