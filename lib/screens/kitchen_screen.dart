...
class _KitchenScreenState extends State<KitchenScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _syncSubscription = DatabaseHelper().syncStreamController.stream.listen((event) {
      if (['orders', 'dishes'].contains(event.table)) {
        AppLogger.info('⚡ KitchenScreen: Real-time update detected for ${event.table}. Refreshing...');
        _loadData();
      }
    });
  ...
  Future<void> _loadData() async {
    ...
    try {
      AppLogger.info('🔄 KitchenScreen: Starting data load...');
      await Future.wait([
        _loadOrdersForDate(),
        _loadProductionQueue(),
        _loadReadyQueue(),
      ]);
      AppLogger.success('✅ KitchenScreen: Data load complete. Orders: ${_orders.length}, ProductionQueue: ${_productionQueue.length}, ReadyQueue: ${_readyQueue.length}');
    } catch (e, stack) {
      AppLogger.error('❌ KitchenScreen: Data load failed: $e');
      print(stack);
    }
    if (mounted) {
      AppLogger.success('✅ KitchenScreen: Setting isLoading = false');
      setState(() => _isLoading = false);
    } else {
      AppLogger.warning('⚠️ KitchenScreen: Not mounted, skipping setState');
    }
  }
  ...
  Widget ordersList(Map<String, List<Map<String, dynamic>>> grouped) {
    ...
    final sections = [
      if (grouped['Breakfast']!.isNotEmpty) ...grouped['Breakfast']!.map((o) => _buildOrderCard(o)),
      if (grouped['Lunch']!.isNotEmpty) ...grouped['Lunch']!.map((o) => _buildOrderCard(o)),
      if (grouped['Dinner']!.isNotEmpty) ...grouped['Dinner']!.map((o) => _buildOrderCard(o)),
      if (grouped['Other']!.isNotEmpty) ...grouped['Other']!.map((o) => _buildOrderCard(o)),
    ];
        ...
        if (isWide) {
          ...
          return SingleChildScrollView(
            ...
            child: Wrap(
              spacing: 12,
              runSpacing: 12,
              children: sections.map((card) => SizedBox(width: itemWidth, child: card)).toList(),
            ),
          );
        }
        return ListView(
      ...
      children: sortedDates.expand((date) {
        return [
          ...
          ...grouped[date]!.map((d) {
            ...
            return Semantics(
              label: '${AppLocalizations.of(context)!.productionQueue}: $dishName for ${d['customerName']}, $pax quantities',
              child: Card(
                elevation: 3,
                margin: const EdgeInsets.only(bottom: 12),
                child: ExpansionTile(
                  ...
                  children: [
                    FutureBuilder<List<Map<String, dynamic>>>(
                      future: InventoryRepository().getRecipeForDishByName(dishName, pax),
