...
class _AllotmentScreenState extends State<AllotmentScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadData() async {
    ...
    _mrpOutput = await InventoryRepository().getMrpOutputForAllotment(widget.mrpRunId);
    _suppliers = await FinanceRepository().getAllSuppliers(widget.firmId);
    
    // Load Service Requirements
    _serviceOrders = await OperationRepository().getServiceRequirementsForMrpRun(widget.mrpRunId);
    
    // Load Event Subcontractors
    final allSubs = await FinanceRepository().getAllSubcontractors(widget.firmId);
    _eventSubcontractors = allSubs.where((s) => (s['category'] ?? '') == 'EVENT').toList();

    // Load already-released POs for this MRP run (for Summary tab)
    _releasedPOs = await FinanceRepository().getPurchaseOrdersByMrpRun(widget.mrpRunId);
    
    // Restore existing allocations from database
    final existingAllocations = await InventoryRepository().getExistingAllocations(widget.mrpRunId);
    _allocations.clear();
  ...
  Future<void> _generatePOs() async {
    try {
      ...
      for (var entry in supplierGroups.entries) {
        ...
        final poNumber = await FinanceRepository().generatePoNumber(widget.firmId);
        final poId = await FinanceRepository().createPurchaseOrder({
        ...
        await FinanceRepository().addPoItems(poId as int, items.map((i) {
          final rate = (i['rate'] as num?)?.toDouble() ?? 0;
          final qty = (i['requiredQty'] as num?)?.toDouble() ?? 0;
          return {
            'itemId': i['ingredientId'] as int,
      ...
      await InventoryRepository().updateOrderStatusIfAllItemsPOd(widget.mrpRunId);
      ...
      AppLogger.info('ERROR Generating POs: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
                      ...
                      trailing: SizedBox(
                        ...
                        child: DropdownButtonFormField<int>(
                          ...
                          onChanged: (v) async {
                            ...
                            await InventoryRepository().updateMrpOutputAllocations(
                              widget.mrpRunId, 
                              {ingredientId: v},
                            );
                          },
                  ...
                  children: [
                    if (needsService)
                      _buildAssignmentRow(
                        ...
                        onChanged: (v) async {
                           await OperationRepository().updateOrderServiceAssignment(
