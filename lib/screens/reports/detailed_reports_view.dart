...
class DetailedReportsView extends StatefulWidget {
  ...
  @override
  State<DetailedReportsView> createState() => _DetailedReportsViewState();
}

class _DetailedReportsViewState extends State<DetailedReportsView> {
  ...
  final Map<String, List<Map<String, dynamic>>> _drillDownData = {};
  ...
  @override
  void didUpdateWidget(covariant DetailedReportsView oldWidget) {
      ...
      switch (_selectedCategory) {
        case 'Orders':
          switch (_selectedSubReport) {
            case 'Summary':
              data = await orderRepo.getOrderStatusReport(
                  _startDate, _endDate, firmId);
              break;
            case 'By Food Type':
              data = await orderRepo.getOrdersByFoodTypeReport(
                  _startDate, _endDate, firmId);
              break;
            case 'By Meal Type':
              data = await orderRepo.getOrdersByMealTypeReport(
                  _startDate, _endDate, firmId);
              break;
            case 'By Time Slot':
              data = await orderRepo.getDeliveryTimeReport(
                  _startDate, _endDate, firmId);
              break;
            case 'Top Locations':
              data = await orderRepo.getRevenueByLocationReport(
                  _startDate, _endDate, firmId);
              break;
          }
          break;
        ...
        case 'Kitchen':
          switch (_selectedSubReport) {
            case 'Production':
              data = await orderRepo.getKitchenProductionReport(
                  _startDate, _endDate, firmId);
              break;
            case 'Top Dishes':
              data = await orderRepo.getTopDishesReport(
                  _startDate, _endDate, firmId);
              break;
            case 'By Category':
              data = await orderRepo.getDishesByCategoryReport(
                  _startDate, _endDate, firmId);
              break;
          }
          break;
        ...
        case 'Dispatch':
          switch (_selectedSubReport) {
            case 'Delivery Status':
              data =
                  await opRepo.getDispatchReport(_startDate, _endDate, firmId);
              break;
            case 'Capacity':
              data = await orderRepo.getDailyCapacityReport(
                  _startDate, _endDate, firmId);
              break;
          }
          break;
        ...
        case 'HR':
          switch (_selectedSubReport) {
            case 'Attendance':
              data = await opRepo.getHRAttendanceReport(
                  _startDate, _endDate, firmId);
              break;
            case 'Overtime':
              data = await opRepo.getHROvertimeReport(
                  _startDate, _endDate, firmId);
              break;
          }
          break;
    ...
    switch (_selectedCategory) {
      case 'Orders':
        switch (_selectedSubReport) {
          ...
          case 'By Food Type':
            return ['Food Type', 'Order Count', 'Total Pax', 'Revenue'];
          case 'By Meal Type':
            return ['Meal Type', 'Order Count', 'Total Pax', 'Revenue'];
          case 'By Time Slot':
            return ['Time Slot', 'Order Count', 'Total Pax'];
          case 'Top Locations':
            return ['Location', 'Order Count', 'Total Pax', 'Revenue'];
        }
        break;
      case 'Kitchen':
        switch (_selectedSubReport) {
          ...
          case 'By Category':
            return ['Category', 'Dish Count', 'Revenue'];
        }
        break;
      case 'Dispatch':
        switch (_selectedSubReport) {
          ...
          case 'Capacity':
            return ['Date', 'Total Pax', 'Veg', 'Non-Veg', 'Orders'];
        }
        break;
      case 'HR':
        switch (_selectedSubReport) {
          ...
          case 'Overtime':
            return ['Staff Name', 'Overtime Hours', 'OT Pay'];
        }
        break;
    }
    return [];
  ...
  List<List<dynamic>> _getExportRows() {
    ...
    for (var index = 0; index < _reportData.length; index++) {
          ...
          switch (_selectedSubReport) {
            case 'Attendance':
              parentRow = [
                ...
                item['totalHours'],
                item['totalOvertime'],
                item['geoFenceCompliant']
              ];
              break;
            case 'Overtime':
              parentRow = [item['name'], item['totalOT'], item['otPay']];
              break;
          }
          break;
      }
      ...
      if (_selectedCategory == 'Orders' &&
          _selectedSubReport == 'Summary' &&
          _expandedItems.contains(date)) {
        if (_drillDownData.containsKey(date)) {
          for (var order in _drillDownData[date]!) {
            ...
            if (_expandedItems.contains(orderKey) &&
                _drillDownData.containsKey(orderKey)) {
      ...
      } else if (_selectedCategory == 'Kitchen' &&
          _selectedSubReport == 'Production' &&
          _expandedItems.contains(date)) {
        final key = 'kitchen_$date';
        if (_drillDownData.containsKey(key)) {
          for (var dish in _drillDownData[key]!) {
            final status =
                dish['orderStatus'] == 'Completed' ? 'Completed' : 'Pending';
            rows.add(['', dish['name'], status, '', '', dish['pax'] ?? 0]);
          }
        }
      } else if (_selectedCategory == 'Dispatch' &&
          _selectedSubReport == 'Delivery Status' &&
          _expandedItems.contains(date)) {
            ...
            child: Column(
              ...
              children: [
                ...
                SingleChildScrollView(
                  ...
                  child: Row(
                    children: ['Orders', 'Kitchen', 'Dispatch', 'HR', 'Finance']
                        .map((cat) {
                          ...
                          onSelected: (val) {
                            if (val) {
                              setState(() {
                                ...
                                _selectedSubReport = _subReports[cat]!.first;
                              });
                              _loadReportData();
                            }
                          },
              ...
              children: [
                ...
                _buildFinanceTile(
                  ...
                  () => Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (_) => const EventProfitabilityScreen())),
                ),
                _buildFinanceTile(
                  'Salary',
                  'Disbursement',
                  ...
                  () => Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (_) => const SalaryDisbursementScreen())),
                ),
              ],
            ),
          ],
  ...
  Widget _buildFinanceTile(String title, String subtitle, IconData icon,
  ...
  Widget _buildOrdersReport() {
    switch (_selectedSubReport) {
      case 'Summary':
        return _buildOrderSummaryReport();
      case 'By Food Type':
        return _buildFoodTypeReport();
      case 'By Meal Type':
        return _buildMealTypeReport();
      case 'By Time Slot':
        return _buildTimeSlotReport();
      case 'Top Locations':
        return _buildLocationReport();
      default:
        return const SizedBox();
    }
  }
  ...
  Widget _buildOrderSummaryReport() {
    ...
    final allKeys = _reportData
        ...
        .toSet();
    final allExpanded =
        _reportData.isNotEmpty && _expandedItems.containsAll(allKeys);

    return Column(
      children: [
        _buildSummaryHeader([
          _SummaryItem(AppLocalizations.of(context).totalOrders,
              totalOrders.toString(), Icons.receipt_long, Colors.blue),
          _SummaryItem(AppLocalizations.of(context).confirmed,
              totalConfirmed.toString(), Icons.pending, Colors.orange),
          _SummaryItem(AppLocalizations.of(context).completed,
              totalCompleted.toString(), Icons.check_circle, Colors.green),
          _SummaryItem(AppLocalizations.of(context).cancelled,
              totalCancelled.toString(), Icons.cancel, Colors.red),
        ]),
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
          child: Row(
            children: [
              Text(AppLocalizations.of(context).totalPax(totalPax),
                  style: const TextStyle(fontWeight: FontWeight.bold)),
              const SizedBox(width: 16),
              Text(
                  '${AppLocalizations.of(context).revenue}: ₹${totalRevenue.toStringAsFixed(0)}',
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                ...
                child: ExpansionTile(
                  ...
                  subtitle:
                      Text('$orders orders | $pax pax | $completed completed'),
                  trailing: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                  ...
                  children: [
                    if (_drillDownData.containsKey(date))
                      ..._drillDownData[date]!.map((order) {
                        ...
                        final isOrderExpanded =
                            _expandedItems.contains(orderKey);
                        ...
                        return ExpansionTile(
                          ...
                          subtitle: Column(
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  Icon(Icons.restaurant_menu,
                                      size: 12, color: Colors.grey.shade600),
                                  const SizedBox(width: 4),
                                  Text('${order['mealType'] ?? 'N/A'}',
                                      style: const TextStyle(fontSize: 11)),
                                ],
                              ),
                              if (order['venue'] != null &&
                                  order['venue'].toString().isNotEmpty)
                                Padding(
                              ...
                              if (order['mobile'] != null &&
                                  order['mobile'].toString().isNotEmpty)
                                Padding(
                                  padding: const EdgeInsets.only(top: 2),
                                  child: Row(
                                    children: [
                          ...
                          trailing: Column(
                            ...
                            children: [
                              Text(
                                  '₹${(order['finalAmount'] ?? order['grandTotal'] ?? 0).toStringAsFixed(0)}',
                                  style: TextStyle(
                              ...
                              Container(
                                ...
                                child: Column(
                                  children: [
                                    ...
                                    ..._drillDownData[orderKey]!.map((dish) =>
                                        Padding(
                                          padding: const EdgeInsets.symmetric(
                                              vertical: 2),
                                          child: Row(
                    ...
                    if (_drillDownData.containsKey(typeKey))
                      ..._drillDownData[typeKey]!.map((order) {
                        ...
                        return ExpansionTile(
                          ...
                          subtitle: Text(
                              '${order['date']} | ${order['totalPax'] ?? 0} pax | ${order['venue'] ?? 'N/A'}',
                              style: const TextStyle(fontSize: 11)),
                          trailing: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              Text(
                                  '₹${(order['finalAmount'] ?? 0).toStringAsFixed(0)}',
                                  style: const TextStyle(
                                      color: Colors.green,
                                      fontWeight: FontWeight.bold,
                                      fontSize: 12)),
                          ...
                          children: [
                            if (_drillDownData.containsKey(orderKey))
                              Container(
                                ...
                                padding: const EdgeInsets.all(12),
                                child: Column(
                                  children: _drillDownData[orderKey]!
                                      .map((dish) => Padding(
                                            padding: const EdgeInsets.symmetric(
                                                vertical: 2),
                                            child: Row(
  ...
  Widget _buildProductionReport() {
    ...
    return Column(
      children: [
        _buildSummaryHeader([
          _SummaryItem('Total Dishes', totalDishes.toString(), Icons.restaurant,
              Colors.blue),
          _SummaryItem(AppLocalizations.of(context).completed,
              completed.toString(), Icons.check_circle, Colors.green),
          _SummaryItem(AppLocalizations.of(context).inProgress,
              inProgress.toString(), Icons.pending, Colors.orange),
          _SummaryItem(AppLocalizations.of(context).pending, pending.toString(),
                      ...
                      Container(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (ctx, i) {
                            ...
                            final status = dish['orderStatus'] == 'Completed'
                                ? 'Completed'
                                : 'Pending';
                            return ListTile(
                              dense: true,
                              ...
                              subtitle: Text(
                                  '${dish['customerName'] ?? 'Unknown'} | ${dish['pax'] ?? 0} pax'),
                              trailing: Chip(
                                label: Text(status,
                                    style: const TextStyle(fontSize: 10)),
                      ...
                      ..._drillDownData[dishKey]!.map((ing) => Padding(
                            ...
                            child: Row(
                              children: [
                                ...
                                Expanded(
                                    child: Text(
                                        ing['ingredientName'] ?? 'Unknown',
                                        style: const TextStyle(fontSize: 12))),
                                Text(
                                    '${(ing['scaledQuantity'] as num?)?.toStringAsFixed(2) ?? '?'} ${ing['unit'] ?? 'kg'}',
                                    style: TextStyle(
                                        fontSize: 11,
  ...
  Future<void> _loadIngredientsForDish(String dishName, String key) async {
    ...
    final ingredients =
        await InventoryRepository().getRecipeForDishByName(dishName, 1);
    if (mounted) {
      setState(() {
  ...
  Widget _buildDispatchReport() {
    switch (_selectedSubReport) {
      case 'Delivery Status':
        return _buildDeliveryStatusReport();
      case 'Capacity':
        return _buildCapacityReport();
      default:
        return const SizedBox();
    }
  }
  ...
  Widget _buildDeliveryStatusReport() {
    ...
    return Column(
      children: [
        _buildSummaryHeader([
          _SummaryItem(AppLocalizations.of(context).totalDispatches,
              totalDispatches.toString(), Icons.local_shipping, Colors.blue),
          _SummaryItem(AppLocalizations.of(context).delivered,
              delivered.toString(), Icons.check_circle, Colors.green),
          _SummaryItem(AppLocalizations.of(context).inTransit,
              inTransit.toString(), Icons.directions_car, Colors.orange),
          _SummaryItem(AppLocalizations.of(context).pending,
              pendingD.toString(), Icons.pending, Colors.grey),
        ]),
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
          child: Row(
                    ...
                    if (_drillDownData.containsKey('dispatch_$date'))
                      Container(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (ctx, i) {
                            final dispatch =
                                _drillDownData['dispatch_$date']![i];
                            final status = dispatch['status'] ?? 'PENDING';
                            return ListTile(
                              dense: true,
                              title: Text('Dispatch #${dispatch['id']}'),
                              subtitle: Text(
                                  '${dispatch['customerName'] ?? 'Unknown'} | ${dispatch['location'] ?? 'N/A'}'),
  ...
  Future<void> _loadDispatchDetails(String date) async {
    final key = 'dispatch_$date';
    if (_drillDownData.containsKey(key)) return;
    final dispatches = await OperationRepository().getDispatchesForDate(date);
    if (mounted) {
      setState(() {
  ...
  Widget _buildCapacityReport() {
    ...
    return Column(
      children: [
        _buildSummaryHeader([
          _SummaryItem(
              'Total Pax', totalPax.toString(), Icons.people, Colors.blue),
          _SummaryItem('Veg', vegPax.toString(), Icons.eco, Colors.green),
          _SummaryItem(
              'Non-Veg', nonVegPax.toString(), Icons.restaurant, Colors.red),
          _SummaryItem(
              'Orders', totalOrders.toString(), Icons.receipt, Colors.purple),
        ]),
        Expanded(
          child: ListView.builder(
            itemCount: _reportData.length,
  ...
  Widget _buildHRReport() {
    switch (_selectedSubReport) {
      case 'Attendance':
        return _buildHRAttendanceReport();
      case 'Overtime':
        return _buildHROvertimeReport();
      default:
        return const SizedBox();
    }
  }
  ...
  Widget _buildHRAttendanceReport() {
    ...
    return Column(
      children: [
        _buildSummaryHeader([
          _SummaryItem(AppLocalizations.of(context).staff,
              staffCount.toString(), Icons.people, Colors.purple),
          _SummaryItem(AppLocalizations.of(context).attendance,
              totalDays.toString(), Icons.calendar_today, Colors.blue),
          _SummaryItem(AppLocalizations.of(context).hours,
              totalHours.toStringAsFixed(0), Icons.access_time, Colors.green),
          _SummaryItem(AppLocalizations.of(context).overtime,
              totalOT.toStringAsFixed(1), Icons.more_time, Colors.orange),
        ]),
        Expanded(
          child: ListView.builder(
            itemCount: _reportData.length,
            itemBuilder: (context, index) {
              ...
              final geoCompliant =
                  (item['geoFenceCompliant'] as num?)?.toInt() ?? 0;
              ...
              return Card(
                ...
                child: ExpansionTile(
                  ...
                  title: Text(name,
                      style: const TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: Text(
                      '$days days | ${hours.toStringAsFixed(1)}h${ot > 0 ? ' (+${ot.toStringAsFixed(1)} OT)' : ''}'),
                  trailing: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      if (geoCompliant > 0)
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (_drillDownData.containsKey(staffKey))
                            ..._drillDownData[staffKey]!.take(10).map((att) =>
                                Padding(
                                  padding:
                                  ...
                                  child: Row(
                                    children: [
                                      ...
                                      const SizedBox(width: 8),
                                      Text(
                                          '${(att['hoursWorked'] as num?)?.toStringAsFixed(1) ?? '0'}h',
                                          style: TextStyle(
                                              fontSize: 11,
                                              fontWeight: FontWeight.bold,
                                              color: Colors.green.shade700)),
                            ...
                            const Center(
                                child: SizedBox(
                                    ...
                                    child: CircularProgressIndicator(
                                        strokeWidth: 2))),
                          if (_drillDownData.containsKey(staffKey) &&
                              _drillDownData[staffKey]!.length > 10)
                            Padding(
                              padding: const EdgeInsets.only(top: 8),
                              child: Text(
                                  '...and ${_drillDownData[staffKey]!.length - 10} more records',
                                  style: TextStyle(
                                      fontSize: 10,
  ...
  Future<void> _loadAttendanceForStaff(dynamic staffId, String key) async {
    ...
    final attendance = await OperationRepository().getAttendanceForStaff(
        staffId is int ? staffId : int.tryParse(staffId.toString()) ?? 0,
        _customStartDate,
        _customEndDate);
    if (mounted) {
      setState(() {
  ...
  Widget _buildHROvertimeReport() {
    ...
    return Column(
      children: [
        _buildSummaryHeader([
          _SummaryItem(AppLocalizations.of(context).staffWithOt,
              _reportData.length.toString(), Icons.people, Colors.orange),
          _SummaryItem(AppLocalizations.of(context).totalOt,
              '${totalOT.toStringAsFixed(1)}h', Icons.more_time, Colors.blue),
          _SummaryItem(
              AppLocalizations.of(context).otPay,
              '₹${totalOTPay.toStringAsFixed(0)}',
              Icons.payments,
              Colors.green),
        ]),
        Expanded(
                      ...
                      child: ListTile(
                        ...
                        title: Text(name,
                            style:
                                const TextStyle(fontWeight: FontWeight.bold)),
                        subtitle:
                            Text('${ot.toStringAsFixed(1)} hours overtime'),
  ...
  Widget _buildPieListReport({
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final color =
            colors?[label] ?? Colors.primaries[index % Colors.primaries.length];
          ...
          child: ListTile(
            ...
            trailing: Column(
              ...
              children: [
                Text('$value',
                    style: const TextStyle(fontWeight: FontWeight.bold)),
                Text('₹${secondary.toStringAsFixed(0)}',
