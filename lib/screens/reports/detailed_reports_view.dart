...
class _DetailedReportsViewState extends State<DetailedReportsView> {
      ...
      switch (_selectedCategory) {
        case 'Orders':
          switch (_selectedSubReport) {
            case 'Summary':
              data = await orderRepo.getOrderStatusReport(_startDate, _endDate, firmId);
              break;
            case 'By Food Type':
              data = await orderRepo.getOrdersByFoodTypeReport(_startDate, _endDate, firmId);
              break;
            case 'By Meal Type':
              data = await orderRepo.getOrdersByMealTypeReport(_startDate, _endDate, firmId);
              break;
            case 'By Time Slot':
              data = await orderRepo.getDeliveryTimeReport(_startDate, _endDate, firmId);
              break;
            case 'Top Locations':
              data = await orderRepo.getRevenueByLocationReport(_startDate, _endDate, firmId);
              break;
          }
          break;
        ...
        case 'Kitchen':
          switch (_selectedSubReport) {
            case 'Production':
              data = await orderRepo.getKitchenProductionReport(_startDate, _endDate, firmId);
              break;
            case 'Top Dishes':
              data = await orderRepo.getTopDishesReport(_startDate, _endDate, firmId);
              break;
            case 'By Category':
              data = await orderRepo.getDishesByCategoryReport(_startDate, _endDate, firmId);
              break;
          }
          break;
        ...
        case 'Dispatch':
          switch (_selectedSubReport) {
            case 'Delivery Status':
              data = await opRepo.getDispatchReport(_startDate, _endDate, firmId);
              break;
            case 'Capacity':
              data = await orderRepo.getDailyCapacityReport(_startDate, _endDate, firmId);
              break;
          }
          break;
        ...
        case 'HR':
          switch (_selectedSubReport) {
            case 'Attendance':
              data = await opRepo.getHRAttendanceReport(_startDate, _endDate, firmId);
              break;
            case 'Overtime':
              data = await opRepo.getHROvertimeReport(_startDate, _endDate, firmId);
              break;
          }
          break;
      }
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        ...
                        if (_selectedTimeline != 'Custom') ...[
                           const SizedBox(width: 8),
                           Text('$_startDate to $_endDate', style: TextStyle(fontSize: 11, color: Colors.grey.shade600)),
                        ],
                      ],
                    ),
                          ...
                          children: [
                            Text(
                              ...
                              style: TextStyle(
                                fontSize: 13,
                                fontWeight: isSelected ? FontWeight.bold : FontWeight.normal,
                                color: isSelected ? _getCategoryColor(_selectedCategory) : Colors.grey,
                              ),
                            ),
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                color: isSelected ? _getCategoryColor(_selectedCategory) : Colors.transparent,
  ...
  Future<void> _loadIngredientsForDish(String dishName, String key) async {
    ...
    final ingredients = await InventoryRepository().getRecipeForDishByName(dishName, 1);
    if (mounted) {
      setState(() {
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  subtitle: Row(
                    children: [
                      Chip(label: Text('🥗 $veg'), backgroundColor: Colors.green.withValues(alpha: 0.2)),
                      const SizedBox(width: 4),
                      Chip(label: Text('🍖 $nonVeg'), backgroundColor: Colors.red.withValues(alpha: 0.2)),
                    ],
                  ),
      ...
      child: Wrap(
        ...
        children: items.map((item) => Column(
          ...
          children: [
            ...
            Text(item.label, style: TextStyle(fontSize: 10, color: Colors.grey.shade600)),
          ],
        )).toList(),
      ),
    );
