...
class _DriverDispatchDetailScreenState extends State<DriverDispatchDetailScreen> {
  ...
  void initState() {
    super.initState();
    AppLogger.info("📍 DriverDispatchDetailScreen: initState start for dispatch ${widget.dispatch['id']}");
    _dispatch = widget.dispatch;
    _loadDetails();
    AppLogger.info("📍 DriverDispatchDetailScreen: initState end");
  }

  Future<void> _loadDetails() async {
    AppLogger.info("📍 DriverDispatchDetailScreen: _loadDetails start");
    setState(() => _isLoading = true);
    
    try {
      final db = await DatabaseHelper().database;
      AppLogger.info("📍 DriverDispatchDetailScreen: DB initialized");
      final orderId = _dispatch['orderId'];
      AppLogger.info("📍 DriverDispatchDetailScreen: Loading details for order $orderId");
      
      // Get order details
      final orders = await db.query('orders', where: 'id = ?', whereArgs: [orderId]);
      if (orders.isNotEmpty) {
        _order = Map<String, dynamic>.from(orders.first);
        AppLogger.info("📍 DriverDispatchDetailScreen: Order loaded: ${_order['id']}");
      }
      ...
      AppLogger.info("📍 DriverDispatchDetailScreen: Dishes loaded: ${_dishes.length}");
      ...
      AppLogger.info("📍 DriverDispatchDetailScreen: Items loaded: records=${_dispatchItems.length}, utensils=${_utensils.length}");
      
      if (mounted) setState(() => _isLoading = false);
      AppLogger.info("📍 DriverDispatchDetailScreen: _loadDetails complete, loading=false");
    } catch (e, stack) {
      AppLogger.info("📍 DriverDispatchDetailScreen error: $e");
      print(stack);
      if (mounted) setState(() => _isLoading = false);
    }
  }
  ...
  Future<void> _acceptAndStart() async {
    ...
    if (confirmed == true) {
      await DatabaseHelper().updateDispatch(_dispatch['id'] as int, {
      ...
      try {
        ...
        if (userId.isNotEmpty) {
           final dbRead = await DatabaseHelper().database;
           final users = await dbRead.query('users', where: 'userId = ?', whereArgs: [userId]);
           if (users.isNotEmpty) {
        ...
        final trackingUrl = 'https://ruchiserv.in/track/${_dispatch['id']}';
        ...
        AppLogger.info("🚀 Dispatch Accepted notification pushed to SQS");
