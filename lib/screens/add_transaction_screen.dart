...
class _AddTransactionScreenState extends State<AddTransactionScreen> {
  ...
  // Party Selection
  String _partyType =
      'Other'; // Staff, Supplier, Customer, Subcontractor, Other
    ...
    try {
      ...
      final Map<String, dynamic> txn = {
        ...
        'description': desc.isNotEmpty
            ? '$desc (Paid to: $partyName)'
            : 'Paid to: $partyName',
        'mode': _paymentMode,
        'createdAt': DateTime.now().toIso8601String(),
        // New Fields
        'relatedEntityType':
            _partyType == 'Other' ? null : _partyType.toUpperCase(),
          ...
          child: Column(
            ...
            children: [
                  ...
                  child: InvoiceScannerWidget(
                    onScanComplete: (result) {
                      setState(() {
                        ...
                        if (result['date'] != null &&
                            result['date'].toString().isNotEmpty) {
                          try {
                        ...
                        _categoryCtrl.text =
                            'Purchase'; // Default for scanned invoices
                      });
                    },
                  ),
                ),
              ...
              DropdownButtonFormField<String>(
                ...
                decoration: const InputDecoration(
                    labelText: 'Party Type', border: OutlineInputBorder()),
                items: _partyTypes
                    .map((e) => DropdownMenuItem(value: e, child: Text(e)))
                    .toList(),
    ...
    return Autocomplete<Map<String, dynamic>>(
      optionsBuilder: (textEditingValue) async {
        ...
        if (_partyType == 'Staff') {
          ...
          list = await FinanceRepository()
              .getAllSuppliers('DEFAULT'); // Using default for now
      ...
      fieldViewBuilder: (context, controller, focusNode, onEditingComplete) {
        return TextFormField(
          ...
          decoration: InputDecoration(
            labelText:
                'Select ${_partyType == "Other" ? "Party Name" : "$_partyType Name"}',
            border: const OutlineInputBorder(),
            suffixIcon: const Icon(Icons.search),
          ),
          validator: (val) {
            if (_partyType != 'Other' && (val == null || val.isEmpty))
              return 'Required for $_partyType';
            return null;
          },
          onChanged: (val) {
            _partyCtrl.text = val;
