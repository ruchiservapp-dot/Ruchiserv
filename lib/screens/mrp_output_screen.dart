...
class MrpOutputScreen extends StatefulWidget {
  ...
  const MrpOutputScreen(
      {super.key, required this.mrpRunId, required this.firmId});
...
class _MrpOutputScreenState extends State<MrpOutputScreen> {
  ...
  Future<void> _loadData() async {
    ...
    final runs = await db
        .query('mrp_runs', where: 'id = ?', whereArgs: [widget.mrpRunId]);
    if (runs.isNotEmpty) {
      _runInfo = runs.first;
    }
                  ...
                  children: [
                    ...
                    Text(
                        AppLocalizations.of(context)
                            .ingredientsCount(_output.length),
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final totalQty = items.fold<double>(
                              0, (sum, i) => sum + (i['requiredQty'] ?? 0));
                          final totalCatCost = items.fold<double>(
                              0, (sum, i) => sum + (i['totalCost'] ?? 0));
                          ...
                          return ExpansionTile(
                            title: Text(category,
                                style: const TextStyle(
                                    fontWeight: FontWeight.bold)),
                            subtitle: Text(
                                '${AppLocalizations.of(context).itemsCount(items.length)} • ₹${totalCatCost.toStringAsFixed(0)}'),
                            trailing: Container(
                            ...
                            children: items.map((item) {
                              final qty =
                                  (item['requiredQty'] as num?)?.toDouble() ??
                                      0;
                              final rate =
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label:
                          Text(AppLocalizations.of(context).proceedToAllotment),
