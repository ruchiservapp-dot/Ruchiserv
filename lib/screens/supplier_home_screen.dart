...
class _SupplierHomeScreenState extends State<SupplierHomeScreen> {
  ...
  Future<void> _loadData() async {
    ...
    if (_supplierId > 0) {
      ...
      ''', [_supplierId]);
      _pendingDeliveries =
          (pendingDeliveries.first['count'] as num?)?.toInt() ?? 0;
            ...
            children: [
              ...
              Expanded(
                child: Container(
                  ...
                  child: Column(
                    children: [
                      Text('₹${(totalAmount / 1000).toStringAsFixed(1)}K',
                          style: const TextStyle(
                              color: Colors.white,
                              fontSize: 24,
                              fontWeight: FontWeight.bold)),
      ...
      children: [
        Expanded(
          child: _notificationCard(
              'New POs', _newPOs, Colors.orange, Icons.notification_important,
              () {
            Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => SupplierPoScreen(
                      supplierId: _supplierId, supplierName: _supplierName),
                )).then((_) => _loadData());
          }),
        ),
        const SizedBox(width: 12),
        Expanded(
          child: _notificationCard(
              'Pending', _pendingDeliveries, Colors.blue, Icons.local_shipping,
              () {
            Navigator.push(
                context,
                MaterialPageRoute(
  ...
  Widget _buildQuickActions() {
    return Row(
      children: [
        Expanded(
            child: _actionCard('POs', Icons.receipt_long, Colors.indigo, () {
          Navigator.push(
              context,
              MaterialPageRoute(
                builder: (_) => SupplierPoScreen(
        ...
        Expanded(
            child:
                _actionCard('Calendar', Icons.calendar_month, Colors.blue, () {
          Navigator.push(
              context,
              MaterialPageRoute(
        ...
        Expanded(
            child: _actionCard(
                'Ledger', Icons.account_balance_wallet, Colors.green, () {
          Navigator.push(
              context,
              MaterialPageRoute(
    ...
    return Column(
      ...
      children: [
        ...
        if (_recentPOs.isEmpty)
          ...
          ...List.generate(
              _recentPOs.length, (i) => _buildPoTile(_recentPOs[i])),
      ],
    );
  }
  ...
  Widget _buildPoTile(Map<String, dynamic> po) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(
            '₹${(po['totalAmount'] as num?)?.toStringAsFixed(0) ?? '0'} • ${po['createdAt']?.toString().substring(0, 10) ?? ''}'),
        trailing: Container(
