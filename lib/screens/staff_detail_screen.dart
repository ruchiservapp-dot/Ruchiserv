...
class _StaffDetailScreenState extends State<StaffDetailScreen> {
  ...
  Future<void> _loadData() async {
    ...
    if (widget.staffId != null) {
      // Load staff details
      final staffList = await OperationRepository().getAllStaff(onlyActive: false);
      final staff = staffList.where((s) => s['id'] == widget.staffId).firstOrNull;
      if (staff != null) {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (_isNew) {
        ...
        await opRepo.insertStaff(data);
      } else {
        await opRepo.updateStaffFields(widget.staffId!, data);
      }
  ...
  Future<void> _deleteStaff() async {
    ...
    if (confirm == true) {
      await OperationRepository().deleteStaff(widget.staffId!);
  ...
  Future<void> _pickPhoto() async {
    ...
    try {
      ...
      // Upload to S3 (with compression + local cache)
      final savedKey = await FileStorageHelper.saveAndUploadImage(
        File(image.path),
        fileType: 'staff_photos',
      );
      
      setState(() => _photoUrl = savedKey);
