...
class _SupplierPoScreenState extends State<SupplierPoScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadPOs() async {
    ...
    final newPOs = await financeRepo.getPurchaseOrders(widget.supplierId.toString(), status: 'SENT');
    
    // For complex filter, we still use the repo for basic fetch or add specialized methods
    final acceptedPOs = await financeRepo.getSupplierPOs(widget.supplierId, status: 'ACCEPTED');
    final dispatchedPOs = await financeRepo.getSupplierPOs(widget.supplierId, status: 'DISPATCHED');
    
    final deliveredPOs = await financeRepo.getSupplierPOs(widget.supplierId, status: 'DELIVERED');
    
    setState(() {
      _newPOs = List<Map<String, dynamic>>.from(newPOs);
      _acceptedPOs = [...List<Map<String, dynamic>>.from(acceptedPOs), ...List<Map<String, dynamic>>.from(dispatchedPOs)];
