...
class _DispatchScreenState extends State<DispatchScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    AppLogger.info("📍 DispatchHubScreen: initState start");
    _tabController = TabController(length: 4, vsync: this);
    _generateDateList();
    _loadAllData();
    _refreshTimer = Timer.periodic(const Duration(seconds: 60), (_) => _loadAllData());
    AppLogger.info("📍 DispatchHubScreen: initState end");
  }
  ...
  Future<void> _loadAllData() async {
    AppLogger.info("📍 DispatchHubScreen: _loadAllData start");
    setState(() => _isLoading = true);
    
    try {
      final dateStr = DateFormat('yyyy-MM-dd').format(_selectedDate);
      AppLogger.info("📍 DispatchHubScreen: Loading for date $dateStr");

      final operationRepo = OperationRepository();
      final orderRepo = OrderRepository();

      // Tab 1: Pending Orders
      final pending = await operationRepo.getDispatchHubPending(dateStr);
      AppLogger.info("📍 DispatchHubScreen: Pending orders loaded: ${pending.length}");

      // Tab 2: Active Dispatches
      final active = await operationRepo.getDispatchHubActive();
      AppLogger.info("📍 DispatchHubScreen: Active dispatches loaded: ${active.length}");

      // Tab 3: Returns
      final returns = await operationRepo.getDispatchHubReturns();
      AppLogger.info("📍 DispatchHubScreen: Returns loaded: ${returns.length}");
      ...
      for (final order in pending) {
        final dishes = await orderRepo.getDishesForOrder(order['id'] as int, firmId);
        pendingWithDishes.add({...order, 'dishes': dishes});
      }
      AppLogger.info("📍 DispatchHubScreen: Dishes for pending orders loaded");
      ...
      AppLogger.info("📍 DispatchHubScreen: _loadAllData complete, loading=false");
    } catch (e, stack) {
      AppLogger.info("📍 DispatchHubScreen error: $e");
      print(stack);
          ...
          return SingleChildScrollView(
            ...
            child: Wrap(
              ...
              children: _pendingOrders
                  .map((o) => SizedBox(width: itemWidth, child: _buildOrderCard(o)))
                  .toList(),
            ),
          );
        }
      ...
      builder: (context, constraints) {
        ...
        if (isWide) {
          ...
          return SingleChildScrollView(
            ...
            child: Wrap(
              spacing: 12,
              runSpacing: 12,
              children: _activeDispatches.map((d) {
                final vehicleNo = d['vehicleNumber'] ?? 'N/A';
                final vehicleType = d['vehicleType'] ?? '';
                final driverName = d['driverName'] ?? 'N/A';
                return SizedBox(width: itemWidth, child: _buildActiveCard(d, vehicleNo, vehicleType, driverName));
              }).toList(),
            ),
          );
        }
        return ListView.builder(
          ...
          itemBuilder: (ctx, i) {
            ...
            final vehicleType = d['vehicleType'] ?? '';
            final driverName = d['driverName'] ?? 'N/A';
            return _buildActiveCard(d, vehicleNo, vehicleType, driverName);
          },
        );
      },
    );
  ...
  Widget _buildActiveCard(Map<String, dynamic> d, String vehicleNo, String vehicleType, String driverName) {
    return Card(
  ...
  void _startDispatch(Map<String, dynamic> order) {
    AppLogger.info("📍 DispatchHubScreen: _startDispatch for order ${order['id']}");
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => _DispatchLoadingSheet(order: order)),
    ).then((_) {
      AppLogger.info("📍 DispatchHubScreen: Returned from _DispatchLoadingSheet");
      _loadAllData();
    });
  }
...
class _DispatchLoadingSheetState extends State<_DispatchLoadingSheet> {
  ...
  Future<void> _loadData() async {
    AppLogger.info("📍 _DispatchLoadingSheet: _loadData start");
    try {
      final operationRepo = OperationRepository();
      final orderRepo = OrderRepository();
      ...
      AppLogger.info("📍 _DispatchLoadingSheet: Utensils loaded: ${dbUtensils.length}");
    ...
    final suggestions = dbUtensils.map((u) => u['name'].toString()).toList();
      ...
      AppLogger.info("📍 _DispatchLoadingSheet: _loadData complete");
    } catch (e, stack) {
      AppLogger.info("📍 _DispatchLoadingSheet error: $e");
      print(stack);
      if (mounted) setState(() => _isLoading = false);
    }
  }
  ...
  Future<void> _completeDispatch() async {
    ...
    try {
      ...
      // Create dispatch (AWS-first)
      final dispatchId = await OperationRepository().insertDispatch({
      ...
      // Insert CUSTOM consumables (AWS-first)
      ...
      // Insert custom utensils (AWS-first)
      for (final item in _customUtensils) {
        ...
        if (name.isNotEmpty && qtyInt > 0) {
          ...
          await OperationRepository().reduceUtensilStock(name, qtyInt);
        }
      }

      // Update order status (AWS-first)
      await OrderRepository().updateOrder(widget.order['id'] as int, {
        'dispatchStatus': 'DISPATCHED',
        'status': 'DISPATCHED', // Optional: sync with order status
        'updatedAt': now,
      }, []);
      ...
      if (!_isCustomerCollection && _selectedVehicleId != null) {
        try {
          ...
          if (gpsEnabled) {
            await LocationService.instance.startTracking(dispatchId ?? 0);
          }
        } catch (e) {
          AppLogger.info('GPS tracking error: $e');
        }
      }
...
// - return_tracking_screen.dart (ReturnTrackingScreen)
// - unload_verify_screen.dart (UnloadVerifyScreen)
